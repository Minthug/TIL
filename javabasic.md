...
개념이 OOP다. 객체 지향적으로 설계된 프로그램은 유지보수가 쉽고 확장성이 
높다는 장점이 있다. 
  <br>
* 함수형 프로그래밍 지원
...
<br>
  ...
  *  자바는 96년에 발표된 언어이고 C++를 개선하는 것 또한 여러 목표 중의 
하나였다. 그래서 자바는 자동으로 메모리를 관리해주는 기능을 추가했다. C, 
C++의 경우 메모리의 생성과 소멸을 개발자가 직접 설계해야 했지만, 자바는 
가비지 컬렉터(Garbage Collector)를 실행시켜 자동으로 사용하지 않는 
메모리를 수거, 이를 통해 개발자는 메모리를 관리하는 수고를 덜고 핵심 
코드에 집중할 수 있게 되었다.
...
언어다
컴파일(Compiler)이란, 특정 프로그래밍 언어를ㄹ 기계가 이해할 수 있는 
언어로 옮기는 번역 과정이다.
...
프로그램이 제대로 실행되기 위해서는 컴퓨터의 다양한 자원(CPU, RAM, 각종 
입출력 장치 등)을 활용해야 하는데, 이런 자원을 관리하는 것은 
운영체제이므로, 프로그램은 운영체제가 정한 규칙을 따라야 한다.
서로 다른 운영체제(Linux, Windows, MacOS 등)에서 프로그램을 실행하기 
위해서는 운영체제별 규칙을 따르는 별도의 절차가 필요한데, JVM은 이 문제를 
해결해준다.

JVM의 경우 JDK나 JRE를 설치하면 자동으로 설치된다. 
자바는 JVM을 거치기 때문에 C나 C++에 비해 속도는 느린 편이지만, JVM 내부의 
최적화된 JIT 컴파일러를 통해 속도를 크게 개선해나가고 있는중이다.
